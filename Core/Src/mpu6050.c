...
uint32_t timer;
//static double gyro_offset_y = 0.0;
Kalman_t KalmanX = {
    .Q_angle = 0.001f,
    .Q_bias = 0.003f,
    .R_measure = 0.03f};
...
uint8_t MPU6050_Init(I2C_HandleTypeDef *I2Cx)
{
    ...
    if (check == 104) // 0x68 will be returned by the sensor if everything goes well
    {
        ...
        // Ghi vào thanh ghi CONFIG (0x1A) giá trị 0x03 (DLPF = 42Hz)
        Data = 0x03;
...
/*void MPU6050_Calibrate_Gyro(I2C_HandleTypeDef *I2Cx, MPU6050_t *DataStruct, uint16_t samples)
{
    ...
    for (uint16_t i = 0; i < samples; i++) {
        ...
        // Cong raw truoc khi tru offset (offset dang = 0 luc nay)
        sumGy += DataStruct->Gyro_Y_RAW / 131.0;
        HAL_Delay(5);
    }
    gyro_offset_y = sumGy / samples;   // luu offset
}
